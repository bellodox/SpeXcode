...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		...
		case "openGlobalKeybindings":
			vscode.commands.executeCommand("workbench.action.openGlobalKeybindings", message.text ?? "spex-code.")
			break
		...
		case "ghostServiceSettings":
			...
			await provider.postStateToWebview()
			vscode.commands.executeCommand("spex-code.autocomplete.reload")
			break
		...
		case "showFeedbackOptions": {
			...
			if (answer === githubIssuesText) {
				await vscode.env.openExternal(vscode.Uri.parse("https://github.com/bellodox/SpeXcode/issues"))
		...
		case "saveApiConfiguration":
			if (message.text && message.apiConfiguration) {
				try {
					...
					await updateGlobalState("listApiConfigMeta", listApiConfig)
					vscode.commands.executeCommand("spex-code.autocomplete.reload") // kilocode_change: Reload autocomplete model when API provider settings change
				} catch (error) {
					provider.log(
